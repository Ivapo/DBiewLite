...
function getDbInfo(fileName: string, fileSize: number): DbInfo {
  ...
  const pageCount = Number(db.exec("PRAGMA page_count")[0]?.values[0]?.[0] ?? 0);
  const pageSize = Number(db.exec("PRAGMA page_size")[0]?.values[0]?.[0] ?? 0);
  const tableCount = Number(
    db.exec("SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%'")[0]
      ?.values[0]?.[0] ?? 0,
  );
  return {
    path: fileName,
...
async function openDatabase(file: File): Promise<void> {
  try {
    ...
    state.indexCount = listIndexes().length;
    ...
    // The tab is named for the file, the browser's echo of a window title.
    document.title = `${file.name} — DBiewLite`;
    render();
  } catch (e) {
    console.error("Failed to open database:", e);
    showStatus(`Could not open ${file.name}: ${e}`);
  }
}
...
/// Quoted the way the Rust side quotes it, and NULL written as an empty field
/// so a file from the demo matches one from the app.
function toCsv(result: QueryResult): string {
  const cell = (val: unknown): string => {
    if (val === null || val === undefined) return "";
    const s = Array.isArray(val) ? `<blob ${val.length} B>` : String(val);
    return /[",\n]/.test(s) ? `"${s.replace(/"/g, '""')}"` : s;
  };
  return [result.columns.map(cell).join(","), ...result.rows.map(r => r.map(cell).join(","))].join("\n");
}
...
async function exportCsv(): Promise<void> {
  if (!state.selectedTable) return;
  try {
    // The whole table in the order being viewed, not the page on screen.
    const all = queryTable(state.selectedTable, Number.MAX_SAFE_INTEGER, 0,
      state.sort?.column ?? null, state.sort?.ascending ?? null);
    download(`${state.selectedTable}.csv`, toCsv(all));
...
async function exportQueryCsv(): Promise<void> {
  ...
  try {
    download("query-results.csv", toCsv(runSqlQuery(sql)));
    showStatus("Exported query-results.csv");
  } catch (e) {
    showStatus(`Export failed: ${e}`);
  }
}
...
function render(): void {
  ...
  if (!state.dbInfo) {
    ...
    // Bound here rather than at startup: the zone is part of this screen, and
    // does not exist until it has been drawn.
    setupDropZone();
    bindHelpOverlay();
    return;
  }
...
function setupKeyboardShortcuts(): void {
  document.addEventListener("keydown", (e) => {
    // There is no native menu in a browser, so the chords it claims on the
    // desktop are handled here.
